...
export const ACTION_TYPES = {
  SEARCH_PRODUCTCATEGORIES: 'productCategory/SEARCH_PRODUCTCATEGORIES',
  ...
  SET_BLOB: 'productCategory/SET_BLOB',
...
export default (state: ProductCategoryState = initialState, action): ProductCategoryState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_PRODUCTCATEGORIES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_PRODUCTCATEGORIES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_PRODUCTCATEGORIES):
      return {
        ...state,
        loading: false,
...
const apiSearchUrl = 'api/_search/product-categories';

// Actions

export const getSearchEntities: ICrudSearchAction<IProductCategory> = query => ({
  type: ACTION_TYPES.SEARCH_PRODUCTCATEGORIES,
  payload: axios.get<IProductCategory>(`${apiSearchUrl}?query=` + query)
});
